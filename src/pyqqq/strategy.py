...
class TradingStrategy:
    ...
    async def _analyze_watchlist(self) -> List[Dict]:
        ...
        for stock in self.watchlist:
            ...
            try:
                ...
                # Claude AI 분석
                analysis = await self.analyzer.analyze_stock(symbol, current_price)
                ...
                logger.info(f"✅ {name}: {signal['recommendation']} (신뢰도: {signal['confidence']:.1%})")
    ...
    async def _execute_buy_order(self, signal: Dict, available_cash: float) -> None:
        ...
        # 신뢰도 필터 (65% 이상만)
        if confidence < 0.65:
            logger.info(f"⏭️  {name} 신뢰도 낮음 ({confidence:.1%}) - 스킵")
            return
