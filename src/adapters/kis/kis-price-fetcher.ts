...
import type { KISMarketDataProvider } from "./kis-market-data-provider.js";

/**
 * KIS price fetcher with caching support.
 *
 * This adapter wraps KISMarketDataProvider and PriceCacheRepository to:
 * 1. Check the cache first (reuse prices < 10 seconds old)
 * 2. If cache miss, fetch from KIS API
 * 3. Store result in D1 cache
 * 4. Return the price
 *
 * This reduces API calls and improves stop-loss/take-profit execution speed.
 */
export class KISPriceFetcher {
  constructor(
    private readonly provider: KISMarketDataProvider,
  ...
  async getPrice(asset: WatchlistAsset, now: Date): Promise<Price | null> {
    ...
    try {
      ...
      const cachedPrice = await this.cache.getCurrentPrice(asset.symbol, "KIS");
      if (cachedPrice) {
      ...
      // Step 2: Cache miss or expired, fetch from KIS API
      ...
      if (snapshot.suspectReasons && snapshot.suspectReasons.length > 0) {
        console.warn(
          `[KISPriceFetcher] Snapshot for ${asset.symbol} is suspect: ${snapshot.suspectReasons.join(", ")}`
        );
      }
      ...
      console.error(`[KISPriceFetcher] Error fetching price for ${asset.symbol}: ${message}`);
      return null;
    }
  }
  ...
  async getPrices(assets: WatchlistAsset[], now: Date): Promise<Map<string, Price>> {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      console.error(`[KISPriceFetcher] Error in getPrices: ${message}`);
      return prices;
    }
  }

  /**
   * Clear the entire KIS price cache (useful for testing/reset).
   */
  async clearCache(): Promise<void> {
    await this.cache.clearBrokerCache("KIS");
  }
}
